...
export default function StakePage() {
  ...
  const stakeAddr = CONTRACTS.mainnet.stakeStaking as `0x${string}`;

  const { data: rewardRate } = useReadContract({
    abi: READ_ABI, address: stakeAddr, functionName: "rewardRate",
    query: { refetchInterval: 3_600_000 },
  });
  const { data: stakedBal } = useReadContract({
    abi: READ_ABI, address: stakeAddr, functionName: "userWeightedStake",
  ...
  const allowanceAmt = BigInt(hNOBTAllowance || "0");
  ...
  const handleApprove = async () => {
    ...
    setBusy(true);
    try {
      await approveHnobt(parsedAmt.toString());
    } catch (e: any) {
      alert(e.message);
    }
    setBusy(false);
  ...
  const handleStake = async () => {
    ...
    setBusy(true);
    try {
      await stakeHnobt(parsedAmt.toString(), duration.lockPeriod);
      setAmount("");
      setModalOpen(false);
    } catch (e: any) {
      alert(e.message);
  ...
  return (
    <div className="max-w-lg mx-auto space-y-4">
      <div className="bg-white rounded-xl border border-emerald-200 p-5 space-y-4">
        <h2 className="text-xl font-bold text-emerald-700">📈 Stake</h2>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setModalOpen(false)}>
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl space-y-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="bg-emerald-50 rounded-lg p-3 space-y-1 text-xs font-mono">
              {([["hNOBT", hNOBT], ["BRT", brt], ["Staking", stakeAddr]] as const).map(([label, addr]) => (
