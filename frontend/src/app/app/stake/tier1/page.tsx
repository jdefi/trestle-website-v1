...
const STAKE_READ_ABI = [
  { inputs: [{ type: "address" }, { type: "uint256" }], name: "users", outputs: [
    ...
    { name: "rewardDebtSnapshotBri", type: "uint256" },
...
export default function Tier1Page() {
  ...
  const { hNOBTBalance, stakeTier1, withdrawTier1, earlyUnstakeTier1 } = useContracts();
  ...
  const tier1Addr = CONTRACTS.mainnet.tier1Staking as `0x${string}`;

  const { data: rewardRate } = useReadContract({
    abi: READ_ABI, address: tier1Addr, functionName: "rewardRate",
    query: { refetchInterval: 3_600_000 },
  });
  const { data: stakedBal } = useReadContract({
    abi: READ_ABI, address: tier1Addr, functionName: "userWeightedStake",
    args: address ? [address] : undefined,
    query: { enabled: !!address, refetchInterval: 3_600_000 },
  });
  const { data: totalStaked } = useReadContract({
    abi: READ_ABI, address: tier1Addr, functionName: "totalWeightedStake",
    query: { refetchInterval: 3_600_000 },
  });
  ...
  const stakes = (rawStakes || [])
    .map((r, i) => {
      ...
      const [amount, weightedAmount, lockEndTime, lockMultiplier, stakeTime, , withdrawn] = r.result as [bigint, bigint, bigint, bigint, bigint, bigint, boolean];
      return { index: i, amount, weightedAmount, lockEndTime, lockMultiplier, stakeTime, withdrawn };
    })
  ...
  const handleStake = async () => {
    ...
    setBusy(true);
    try {
      await stakeTier1((BigInt(Math.floor(parseFloat(amount) * 1e18))).toString(), duration.lockPeriod);
      setAmount("");
      setModalOpen(false);
    } catch (e: any) {
      alert(e.message);
  ...
  return (
    <div className="max-w-lg mx-auto space-y-4">
      <div className="bg-white rounded-xl border border-emerald-200 p-5 space-y-4">
        <h2 className="text-xl font-bold text-emerald-700">📈 Stake: hNOBT Staking</h2>
        <p className="text-sm text-gray-500">Stake hNOBT to earn BroilerPlus (BRT) rewards.</p>
        ...
        <div className="bg-white rounded-lg border p-3 text-sm space-y-1">
          <div className="flex justify-between">
            ...
            <span className="font-semibold text-emerald-600">{fmt((Number(hNOBTBalance || "0") / 1e18).toFixed(4))} hNOBT</span>
          </div>
          {userStaked > 0n && (
            <div className="flex justify-between">
              <span className="text-gray-500">Weighted stake:</span>
              <span className="font-semibold">{fmt((Number(userStaked) / 1e18).toFixed(4))} hNOBT</span>
            </div>
          )}
        </div>
        ...
        >
          {isConnected ? "Stake hNOBT" : "Connect Wallet to Stake"}
        </button>
      </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setModalOpen(false)}>
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl space-y-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-lg">Walkthrough: Stake hNOBT</h3>
              <button onClick={() => setModalOpen(false)} className="text-gray-400 hover:text-gray-600 text-xl">✕</button>
            </div>

            <div className="bg-emerald-50 rounded-lg p-3 space-y-1 text-xs font-mono">
              {([["hNOBT", hNOBT], ["BRT", brt], ["Staking", tier1Addr]] as const).map(([label, addr]) => (
                <button key={label} onClick={() => copyAddr(label, addr)} className="w-full flex justify-between items-center hover:bg-emerald-100/50 rounded px-1 -mx-1 transition">
            ...
            <div className="bg-gray-50 rounded-lg p-3 space-y-1.5 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium text-emerald-600">{fmt(dailyDisplay)} BRT</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Est. pending BRT:</span>
                <span className="font-medium text-emerald-600">{fmt(pendingEstimate)} BRT</span>
              </div>
