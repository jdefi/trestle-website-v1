...
export default function Tier2Page() {
  ...
  const { brtBalance, stakeTier2, withdrawTier2, earlyUnstakeTier2 } = useContracts();
  ...
  const tier2Addr = CONTRACTS.mainnet.tier2Staking as `0x${string}`;
  const lpAddr = CONTRACTS.mainnet.brtLP as `0x${string}`;

  const { data: rewardRate } = useReadContract({
    abi: READ_ABI, address: tier2Addr, functionName: "briRewardRate",
    query: { refetchInterval: 3_600_000 },
  });
  const { data: xgovRateData } = useReadContract({
    abi: READ_ABI, address: tier2Addr, functionName: "xgovPointRate",
    query: { refetchInterval: 3_600_000 },
  });
  const { data: stakedBal } = useReadContract({
    abi: READ_ABI, address: tier2Addr, functionName: "getUserTotalWeightedBalance",
    args: address ? [address] : undefined,
    query: { enabled: !!address, refetchInterval: 3_600_000 },
  });
  const { data: totalStaked } = useReadContract({
    abi: READ_ABI, address: tier2Addr, functionName: "totalWeightedSupply",
    query: { refetchInterval: 3_600_000 },
  });
  const { data: earnedXgov } = useReadContract({
    abi: READ_ABI, address: tier2Addr, functionName: "earnedXgovPoints",
    args: address ? [address] : undefined,
    query: { enabled: !!address, refetchInterval: 3_600_000 },
  });
  ...
  const handleStake = async () => {
    ...
    setBusy(true);
    try {
      await stakeTier2((BigInt(Math.floor(parseFloat(amount) * 1e18))).toString(), duration.lockPeriod);
      setAmount("");
      setModalOpen(false);
    } catch (e: any) {
      alert(e.message);
  ...
  return (
    <div className="max-w-lg mx-auto space-y-4">
      <div className="bg-white rounded-xl border border-blue-200 p-5 space-y-4">
        <h2 className="text-xl font-bold text-blue-700">⛏️ Mine: BRT/WPOL LP Mining</h2>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setModalOpen(false)}>
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl space-y-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="bg-blue-50 rounded-lg p-3 space-y-1 text-xs font-mono">
              {([["BRT LP", brtLP], ["BRT", brt], ["Staking", tier2Addr]] as const).map(([label, addr]) => (
